 ...
 * 将文件通过浏览器进行上传
 */
let getFileContent = function (fileInput, callback) {
 ...
 * 第一次访问网页，本地没有数据时执行
 */
let initData = function (url) {
    console.log("By litcc | https://github.com/litcc/opage    v0.1.1   2019/04/28");
    if (url === null) {
        url = "./data.json";
    }
    let a1 = getLocalData();
 ...
 * 设置数据到localStorage
 */
let setLocalData = function (value) {
